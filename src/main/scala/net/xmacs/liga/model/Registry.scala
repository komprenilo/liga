...
private[liga] object Registry {
  ...
  @throws[ModelResolveException]
  private[liga] def getRegistry(uriOption: Option[String]): Registry = {
    uriOption match {
      case Some(uri) =>
  ...
  /** Prepend the default URI root if the input uri string does not have a scheme specified
    ...
    */
  private[liga] def normalize_uri(uri: String): URI = {
